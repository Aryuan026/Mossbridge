...
if [[ -n "${EXISTING_PID}" ]]; then
  ...
  echo "shared asheriebridge already running pid=${EXISTING_PID}"
  exit 0
fi
...
cd "${ROOT_DIR}"
export ASHERIEBRIDGE_CODEX_ENDPOINT="ws://127.0.0.1:${PORT}"
node ./bin/asheriebridge.js start --checkin &
BRIDGE_PID="$!"
