...
REMOTE_URL="${ASHERIEBRIDGE_CODEX_ENDPOINT:-ws://127.0.0.1:${PORT}}"
STATE_DIR="${ASHERIEBRIDGE_STATE_DIR:-$HOME/.asheriebridge}"
SESSION_FILE="${ASHERIEBRIDGE_SESSIONS_FILE:-${STATE_DIR}/sessions.json}"
WORKSPACE_ROOT="${ASHERIEBRIDGE_WORKSPACE_ROOT:-$PWD}"
ACCOUNT_DIR="${STATE_DIR}/accounts"
RUNTIME_ID="${ASHERIEBRIDGE_RUNTIME:-codex}"
...
exec "${CODEX_COMMAND}" resume "${THREAD_ID}" --remote "${REMOTE_URL}" -C "${RESOLVED_WORKSPACE_ROOT}" "$@"
