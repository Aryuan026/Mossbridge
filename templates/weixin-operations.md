...
If a warm-memory card should stay as a long-term resident anchor across time, mark it explicitly with `pinned: true` or `certainty_state: anchor`. Use that sparingly for relationship symbols, stable identity anchors, and other cards that deserve to remain visible even as newer cards arrive.
