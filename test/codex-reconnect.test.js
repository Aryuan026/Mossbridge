...
test("codex adapter lets configured env model override stored session model", async () => {
  ...
  try {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mossbridge-codex-env-model-"));
...
test("codex adapter enables native image input from model metadata or explicit override", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mossbridge-codex-image-cap-"));
  const sessionsFile = path.join(tempDir, "sessions.json");
  const indexPath = path.resolve(__dirname, "../src/adapters/runtime/codex/index.js");
  delete require.cache[indexPath];
  const { createCodexRuntimeAdapter } = require(indexPath);
...
test("codex adapter skips full opening instructions for system runtime turns", async () => {
  const indexPath = path.resolve(__dirname, "../src/adapters/runtime/codex/index.js");
  const rpcClientPath = path.resolve(__dirname, "../src/adapters/runtime/codex/rpc-client.js");
  const mcpConfigPath = path.resolve(__dirname, "../src/adapters/runtime/codex/mcp-config.js");
  ...
  class MockCodexRpcClient {
    ...
    async startThread() {
      return { result: { thread: { id: `thread-${sent.length + 1}` } } };
    }
    async resumeThread() {
      return { result: {} };
    }
    async sendUserMessage(params) {
      ...
      return { result: { turn: { id: `turn-${sent.length}` } } };
    }
    async close() {}
  }
  ...
  try {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "mossbridge-codex-opening-skip-"));
    const workspaceRoot = path.join(tempDir, "workspace");
    fs.mkdirSync(workspaceRoot);
    const instructionsFile = path.join(tempDir, "weixin-instructions.md");
    fs.writeFileSync(instructionsFile, "STABLE WECHAT RULE", "utf8");
    ...
    assert.match(sent[0], /WECHAT SESSION INSTRUCTIONS/);
    assert.match(sent[0], /STABLE WECHAT RULE/);
    assert.doesNotMatch(sent[1], /WECHAT SESSION INSTRUCTIONS/);
    assert.match(sent[1], /^MOSSBRIDGE WAKE ANCHOR/);
