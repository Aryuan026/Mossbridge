...
| Bridge-owned dreaming | Dreaming may run in a private external process, so standalone Bridge would not整理记忆 when that process is closed. | Shared bridge code owns quiet-window entry, source digest, `dreaming_mutation_log`, and retry state; no private scheduler is required. |
| Dreaming completion gate | A scheduled dreaming item can be touched by the scheduler but still fail before mutation/writeback. | Treat trigger as only an attempt. Mark complete only after runtime completion plus `mossbridge_memory_metabolism_receipt_write`; foreground-active holds, runtime errors, missing receipts, or writeback failures reschedule the same item after a delay. |
| Dreaming receipts | Executor results can be saved internally but disappear from wakeup/run logs. | `memory_metabolism`, warm writes, cold promotions, cold patches, no-op decisions, and failures must be visible in local logs or receipts. |
| Warm-card duplicate metabolism | Dreaming can keep writing near-duplicate warm cards without ever consolidating them into cold structure. | Duplicate/near-duplicate warm cards should surface as a dreaming task, then produce a cold topology/cluster mutation that preserves source material ids and archive/case/episode refs instead of deleting evidence. |
| Dreaming tone | Memory整理 can drift into cold code-review or work-order language. | Prompts may organize facts, but must preserve natural companionship texture and must not become frontend expression rules. |
...
| Context delivery | Resident anchors, warm cards, cold vines, ongoing tracks, hot context, and recent-thread can become every-turn noise. | Delivery should be relevance-gated, session-aware, and able to退场 when a topic is no longer active; delivery diagnostics stay backend-only and are not injected into the runtime prompt. |
| Proactive wakeups | Wakeups can fire without enough recent memory, repeat full session instructions, or fail at WeChat send. | Wakeups must carry warm/ongoing/recent context, use a short system-turn packet instead of full opening instructions, defer/retry send failures, and avoid pretending a裸 reminder is relationship-aware. |
